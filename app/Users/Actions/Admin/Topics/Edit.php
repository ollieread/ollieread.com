...
class Edit extends Action
{
    public function edit(int $id)
    {
        ...
        if ($topic) {
            return $this->response()->view('users.admin.topics.edit', compact('topic'));
        }
    ...
    public function update(int $id, Request $request)
    {
        ...
        if ($topic) {
            ...
            if ((new UpdateTopic)->setTopic($topic)->setInput($input)->perform()) {
                Alerts::success(trans('users.admin.edit.success', ['entity' => trans_choice('entities.topic', 1)]), 'admin.topics');
