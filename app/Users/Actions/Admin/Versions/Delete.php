...
class Delete extends Action
{
    public function delete(int $id)
    {
        ...
        if ($version) {
            return $this->response()->view('users.admin.versions.delete', compact('version'));
        }
    ...
    public function destroy(int $id, Request $request)
    {
        ...
        if ($version) {
            ...
            if ((new DeleteVersion)->setInput($input)->setVersion($version)->perform()) {
                Alerts::success(trans('users.admin.delete.success', ['entity' => trans_choice('entities.version', 1)]), 'admin.versions');
