...
class Edit extends Action
{
    public function edit(int $id)
    {
        ...
        if ($version) {
            return $this->response()->view('users.admin.versions.edit', compact('version'));
        }
    ...
    public function update(int $id, Request $request)
    {
        ...
        if ($version) {
            ...
            if ((new UpdateVersion)->setVersion($version)->setInput($input)->perform()) {
                Alerts::success(trans('users.admin.edit.success', ['entity' => trans_choice('entities.version', 1)]), 'admin.versions');
